...
k = 10;  % if k is too large for selected nwf, the index overlaps and in the end we will end up with less clusters 
...
for i = 1 :n_test_nooutliers
    current_test_idx = test_idx(idxtest_nooutliers(i)) ; 
    for j = 1 : n_train_nooutliers
    current_train_idx = train_idx(idxtrain_nooutliers(j))   ;
