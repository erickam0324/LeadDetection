...
function [S3,CS,NORMfactor] = S3_L1b_read_ericka(FName)
...
ncid = netcdf.open(fullfile(FName,'measurement.nc'),'NC_NOWRITE');
...
for ii = 1:numel(CS.GEO.LAT)
    %Apply threshold retracker to obtain retracking point and Pu
    defval('IDXpoi',[]) 
        [CS.n_ret(ii),CS.Pu(ii)]  = RETRACKER.Threshold_mat(NORMdata(: ,1));
end 
...
% %FNameL2 = 'S3A_SR_1_SRA____20170401T004834_20170401T013904_20170426T154541_3029_016_088______MAR_O_NT_002.SEN3' ; 
% % 
% % %Cornelis' approach in case of 1 L2-file
% % %TMP     = dir(fullfile(PathL2,sprintf('%s*%s',FNameL2(1:55),FNameL2(end-29:end))));
% % TMP     = dir(fullfile(sprintf('%s*%s',FNameL2(1:55),FNameL2(end-29:end))));
% % i       = 0;
% % while isempty(TMP)
% %     i   = i+1;
