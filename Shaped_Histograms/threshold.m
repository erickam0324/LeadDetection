...
% Last Modified by GUIDE v2.5 26-Feb-2008 14:12:57
...
% 	CenterWindow(handles.figMainWindow);		
    ...
    if (strcmp(reply,'Yes') == 1)
        ...
        if isempty(errormessage)
            try
                ...
                msgboxw('Error reading and displaying image file');
            end
		else
            msgboxw(errormessage);
        end
...
%        contents{get(hObject,'Value')} returns selected item from listbox_DisplayColorBand
    ...
    try
        ...
        errormessage = 'Error viewing selected color band';
    end
    if ~isempty(errormessage)
...
function ShowThresholdedBinaryImage(hObject, eventdata, handles)
	...
	if (colorBand ~= 1) || (numberOfColorBands == 1)
        ...
        axes2_image = threshold_grayscale_image(axes1_image, g_MinThreshold, g_MaxThreshold);
		% axes2_image will be a uint8 image with values of 0 and/or 1 only.
...
function PlaceThresholdBar(handles, dblLowThreshold, dblHighThreshold)
	...
	if get(handles.axesHist, 'visible')
		...
		% Erase the old lines.
		hOldLowBar=findobj('type', 'hggroup');
		delete(hOldLowBar);
		hOldHighBar=findobj('type', 'hggroup');
		delete(hOldHighBar);
