...
for SA = 1
    ...
    predictions = fun_DBSCAN_classification(wfSA{SA}.wftosave, 0.001, 10) ; 
    ...
    TrueIce(SA) = sum(predictions_conf == 1 & conf == 1) ; 
    TrueLead(SA) = sum(predictions_conf == 3 & conf == 3) ; 
    FalseIce(SA) = sum(predictions_conf == 1 & conf == 3) ; 
    FalseLead(SA) = sum(predictions_conf == 3 & conf == 1) ; 
    accuracy(SA) = (TrueIce(SA) + TrueLead(SA)) / (TrueIce(SA) + TrueLead(SA) + FalseIce(SA) + FalseLead(SA)) ; 

    TLR(SA) = TrueLead(SA) / (TrueLead(SA) + FalseIce(SA)) ; 
    FLR(SA) = FalseLead(SA) / (FalseLead(SA) + TrueIce(SA)) ; 
    % 
end
